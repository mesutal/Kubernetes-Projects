...
metadata:
  ...
  labels:
    app: container-info
  annotations:
    kubernetes.io/change-cause: deploy/clarus-deploy is set as container-info=example/container-info:1.0
spec:
  replicas: 2
  selector:
    matchLabels:
...
- Describe deployment and note the image of the deployment. In our case, it is example/container-info:1.0.
...
```bash
kubectl set image deploy clarus-deploy container-info=example/container-info:2.0
```
...
...
...
    kubernetes.io/change-cause: kubectl set image deploy clarus-deploy container-info=example/container-info:3.0
...
...
    spec:
      containers:
      - image: example/container-info:3.0
...
...
```
